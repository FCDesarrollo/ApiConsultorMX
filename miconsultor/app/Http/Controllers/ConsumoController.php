...
class ConsumoController extends Controller
{
        ...
        if ($autenticacion[0]['error'] == 0) {
            ...
            for ($i = 0; $i < count($registros); $i++) {
                ...
                if ($registros[$i]["status"] == 1) {
                    ...
                    if (empty($doc)) {
                        DB::table('mc_almdigital_doc')->insertGetId([
                            'idalmdigitaldet' => $idalmacen_det,
                    ...
                    DB::table('mc_almdigital_doc')->where("idalmdigitaldet", $idalmacen_det)->where("iddocadw", $iddoc)->delete();
                }
                ...
                $resp = DB::table('mc_almdigital_det')->where("id", $idalmacen_det)->update([
            ...
            if (!empty($result)) {
                ...
                if (!empty($lote)) {
                    ...
                    if (!empty($existelote)) {
                        ...
                        DB::table('mc_almdigital_det')->where("id", $idDocDig)->update(['idalmdigital' => $lid, 'codigodocumento' => $newar]);

                        DB::table('mc_almdigital')->where("id", $lid)->update(['totalregistros' => $cantReg, 'totalcargados' => $cantCarg]);
                    } else {
                        $now = date('Y-m-d h:i:s A');
                        ...
                        DB::table('mc_almdigital_det')->where("id", $idDocDig)->update(['idalmdigital' => $idU, 'codigodocumento' => $newar]);
                    }
                }

                $userSto = $autenticacion[0]["userstorage"];
                $passSto = $autenticacion[0]["passstorage"];
                $res = $this->MoverDocumento(
                    $userSto,
                    $passSto,
                    $idmenu,
                    $carpIni,
    ...
    public function MoverDocumento(
        String $userSto,
        String $passSto,
        int $idMenu,
        String $carpIni,
